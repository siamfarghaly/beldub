        ...
        {posts.map((blogpost, i) => (
          <div key={i} style={{ maxWidth: '600px', textAlign: 'left' }}>
            <Link style={{ color: 'inherit', textDecoration: 'inherit' }} to={blogpost.slug}>
              ...
              <p style={{ fontWeight: "var(--font-medium)" }}>{blogpost.content.data.content.slice(0, 250).replace(/#\S+/g, "")}<span style={{ textDecoration: 'none', color: 'var(--color-primary)' }}> ...more</span></p>
            </Link>
          </div>
        ))} </div>
    </Layout>
